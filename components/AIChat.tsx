...
import { getMemory, saveConversationMessage } from '../services/memoryService';
...
export const AIChat: React.FC<AIChatProps> = ({ accounts, expenses, income, debts, savings, stats, profile, currentUserId, onExecuteAction, onReceiptScanned }) => {
  // UI State
  ...
  useEffect(() => {
    if (currentUserId && currentUserId !== 'guest') {
      const mem = getMemory(currentUserId);
      if (mem.conversationHistory && mem.conversationHistory.length > 0) {
        setMessages(mem.conversationHistory);
      } else {
        setMessages([{ id: 'init', role: 'model', text: 'I am The Vault Intelligence. How can I assist you today?', timestamp: Date.now() }]);
      }
    }
  }, [currentUserId]);
  ...
  // --- ARTIFACT RENDERING ---
  const handleArtifact = (title: string, code: string) => {
    setArtifact({
  ...
  const addMessage = (msg: ChatMessage) => {
     ...
     // Persist to memory
     saveConversationMessage(currentUserId, msg);
  };
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (userMsg.attachment) {
        ...
        const result = await sendMessageThinking(userMsg.text, contextData, historyText, currentUserId);
        responseText = result.text;
        modelUsed = result.model;
      } else {
        const result = await sendMessageStandard(userMsg.text, contextData, historyText, false, currentUserId);
        ...
        if (result.responseObject?.functionCalls && result.responseObject.functionCalls.length > 0) {
          ...
          for (const call of result.responseObject.functionCalls) {
             if (call.name === 'generateSimulation') {
                handleArtifact(call.args.title as string, call.args.htmlCode as string);
                actionResults.push(`Generated simulation: ${call.args.title}`);
             } else {
                try {
                   const actionResult = await onExecuteAction(call.name, call.args);
                   actionResults.push(`✅ Executed ${call.name}: ${actionResult.message || 'Success'}`);
                } catch (err) {
                   actionResults.push(`❌ Failed to execute ${call.name}: ${String(err)}`);
                }
             }
          }
      ...
      console.error(err);
      addMessage({ id: Date.now().toString(), role: 'model', text: "Error processing request.", timestamp: Date.now() });
    } finally {
      setLoading(false);
      setAiState('idle');
    }
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
            ...
            if (msg.toolCall) {
              ...
              for (const fc of msg.toolCall.functionCalls) {
                if (fc.name === 'generateSimulation') {
                    handleArtifact(fc.args.title as string, fc.args.htmlCode as string);
          ...
          onclose: () => { setIsLiveConnected(false); setMode('standard'); setAiState('idle'); },
          onerror: (err) => { console.error(err); setIsLiveConnected(false); }
        },
        config: {
  ...
  const handleReceiptUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onloadend = async () => {
          ...
          try {
              ...
              addMessage({ id: Date.now().toString(), role: 'model', text: `Scanned receipt from ${data.merchant} for $${data.total}.`, timestamp: Date.now() });
  ...
  return (
    <>
      ...
      {/* --- LIVE VOICE HUD --- */}
      {isLiveConnected && (
        <div className="fixed inset-x-0 bottom-0 p-8 flex flex-col items-center justify-end pointer-events-none z-50">
          <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-full px-8 py-4 flex items-center gap-6 pointer-events-auto animate-[slideUp_0.3s_ease-out]">
             ...
             <button 
               ...
               className="bg-red-500/20 hover:bg-red-500/40 text-red-200 rounded-full p-3 border border-red-500/30 transition-all"
             >
               <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
             </button>
          </div>
          <p className="text-white/50 text-xs mt-4">Speak clearly. I am listening.</p>
        </div>
      )}

      {/* --- STANDARD CHAT TOGGLE BUTTON --- */}
      {!isLiveConnected && (
        <button 
        ...
        <div className={`fixed bottom-24 right-6 w-[90vw] md:w-[400px] bg-black/80 backdrop-blur-xl border border-violet-500/30 rounded-2xl shadow-2xl z-40 transition-all duration-300 flex flex-col overflow-hidden ${isOpen ? 'opacity-100 translate-y-0 h-[600px]' : 'opacity-0 translate-y-10 pointer-events-none h-0'}`}>
            ...
            <form onSubmit={handleSend} className="p-3 bg-black/40 border-t border-white/10 flex gap-2">
                <input 
                    ...
                    className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-1 focus:ring-violet-500"
                />
                <button type="submit" disabled={loading} className="p-2.5 bg-violet-600 text-white rounded-xl hover:bg-violet-500 disabled:opacity-50">
      ...
      <Modal isOpen={!!artifact?.isVisible} onClose={() => setArtifact(null)} title={artifact?.title || "Simulation"}>
          ...
          <div className="mt-4 flex justify-end">
              <button onClick={() => setArtifact(null)} className="px-4 py-2 bg-gray-700 text-white rounded-lg">Close Simulation</button>
          </div>
      </Modal>
    </>
  );
